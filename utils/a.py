...
if __name__ == '__main__':
    ...
    for c in aa[1000:1003]:
        ...
        for i in range(len(strokes_raw)):
            ...
            imageio.imwrite(f'out1\\{character_name}\\{i}.png', np.where(skimage.morphology.skeletonize(np.where(np.max(imageio.imread(png_temp), axis=-1)>0, 1, 0))==True, 0, 1))