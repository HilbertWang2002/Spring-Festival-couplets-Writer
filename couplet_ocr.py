...
API_KEY = "ee98b68d7681ebc35d33e4b90b8eed2d"
...
def couplet_ocr(picFilePath):
    ...
    data = json.loads(s)
    s = ''.join([i['word'][0]['content'] for i in data['data']['block'][0]['line']])
    return s
def ocr_current_camera(cameram):
    if cameram.get_camera_frame('./temp/temp.png'):
        return couplet_ocr('./temp/temp.png')
    else:
        return 'Failed'
